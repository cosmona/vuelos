...
/* 
  ...
  if (!vuelo) {
    ...
    if (errorClase.length === 0) {
      ...
      <p><img src="https://www.dictionary.com/e/wp-content/uploads/2018/03/its-a-trap.gif"></p>`;  //TODO pasar la imagen a IMG
    }
    ...
    //* pinta el logo 
    newArticle.innerHTML += `<article id="cabeceraVuelo${idVuelo}"><p><img class="logo" src="https://daisycon.io/images/airline/?width=300&height=150&color=ffffff&iata=${aerolinea}" alt=""></p><p> ${salida} - ${llegada}</p><p>${escalas}</p><p>${duracion}</p><p>${precio} € </p> </article>`; //! DOC 
    ...
    for (let i = 0; i < segmentos.length; i++) {
      ...
      let aerolinea = segmentos[i].carrierCode; //! NO LO USA?? DUPLICADO LINEA 27? YA NO LO PINTAMOS EN DETALLES?
    ...
    //!---------------------------------------------------------------------------------------
      ...
      if (!aeropuerto[0].error) {
        ...
        //* escucha click en cada (o solo una?) tarjeta del vuelo para plegar/desplegar restricciones covid
